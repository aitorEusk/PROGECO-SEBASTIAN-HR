...
import { s as sql, l as flxSync, u as util, m as msg, C as ConftokenProvider, n as nav } from './conftoken-949aae77.js';
...
(function (dependencies) {
  ...
  async function processPropDependency(isNew, form, prop, conf, props) {
    if (form.length > 0 && prop) {
      for (let i = 0; i < prop.DependingProperties.length; i++) {
        await processDependency(isNew, form, prop.DependingProperties[i], conf.user, props);
      }
    }
  }
  ...
  async function processDependency(isNew, form, dep, tokens, props) {
    ...
    if (isNew || (dependant_property === null || dependant_property === void 0 ? void 0 : dependant_property.DetachedFromDB)) {
      await execValueDependency(form, dep, tokens);
    }
    ...
    if (dep.SQLComboSentence || dep.SQLComboFilter) {
      ...
      if (withValue && sqlSentence) {
        ...
        sql.getTable(sqlSentence).then((tbl) => {
          ...
          if (dependant_property[0].nodeName !== 'FLX-MULTICOMBO' && current_value != null && current_value !== "" && !rows.some(row => row[value_field] == current_value)) {
  ...
  async function execValueDependency(form, dep, tokens) {
    if (dep.SQLValue) {
      ...
      let value = await sql.getValue(parseSQLdependency(dep.SQLValue, form, tokens))
        .catch(() => form_js.is_executing_value_dep = false); //If the dependency fails we avoid form from not getting saved ever
  ...
  async getProperties(values) {
    ...
    for (let i = 0; i < properties.length; i++) {
      ...
      if (properties[i].ControlType == 'separator') {
        ...
        itm.setAttribute('label', properties[i].Label);
  ...
  setSQLValidator(prop) {
    ...
    if (is_flx_element) {
      input_element.sqlValidatorFunction = () => {
        return this.sqlValidatorFunction(prop, input_element, 'ionBlur');
      };
      return;
    }
    ...
    input_element.sqlValidatorFunction = () => {
      return this.sqlValidatorFunction(prop, input_element, blur_type);
    };
  }
  async sqlValidatorFunction(prop, input_element, blurType) {
    let inputs = [];
    ...
    while (sqlValidator.includes("{{") && sqlValidator.includes("}}")) {
      ...
      //If the input has no value we set it as null so the SQL can understand it
      const input_value = document.getElementsByName(sqlValue)[0].value;
      inputs.push(input_value ? input_value : null);
    }
    try {
      const value = await sql.getValue(sqlValidator, inputs);
      ...
      console.error(`Error on validator: ${prop.PropertyName}\n${prop.SQLValidator}`);
      msg.showError(err, true, false);
      throw err;
    }
  }
