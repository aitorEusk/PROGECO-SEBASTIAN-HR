import { i as initialize } from './ionic-global-7b33f09f.js';
import { C as ConftokenProvider, u as util, m as msg, s as sql, n as nav, c as cordova, a as checkAvailability, g as getPromise, I as IonicNativePlugin, _ as __extends$1, b as cordovaPropertyGet, d as cordovaPropertySet, e as Injectable, f as Capacitor, r as registerPlugin, W as Webapi, h as navOnline, i as cam, j as gps, t as tracking, k as util$1, l as flxSync, o as storage, p as ai, z as zebraPrinter, P as Preferences, q as ConfToken, U as UrlConfig, v as authToken, w as storage$1, S as SQLiteConnection, x as CapacitorSQLite, y as ConftokenService, G as GeneralContextProvider } from './conftoken-949aae77.js';
import { j as jquery } from './jquery-254bc370.js';
import { w as whiteboard } from './whiteboard-1d18dc95.js';
...
(function (forms) {
  async function save(btn, showMessage = true, goBack = false) {
    return new Promise(async (resolve, _) => {
      const edit = jquery(btn).closest('flx-edit')[0];
      const form = edit.querySelector('form');
      if (form.is_executing_value_dep) {
        form.save_function = () => { save(btn, showMessage, goBack).then(() => resolve(true)); };
      }
      else {
        let tkn = await ConftokenProvider.config();
        //We check the sql validations, so they get checked even if they weren't modified
        const sqlvalidator_inputs = form.querySelectorAll('[data-msg-sqlvalidator]');
        for (let i = 0; i < sqlvalidator_inputs.length; i++) {
          const input = sqlvalidator_inputs[i];
          if (input.sqlValidatorFunction)
            await input.sqlValidatorFunction();
        }
        //If the form is not valid we show the errors
        const $edit = jquery(edit), $form = jquery(form);
        if (!$form.valid()) {
          ...
          $edit.find('[data-msg-max]').blur();
          $edit.find('[data-msg-min]').blur();
          //In certain specif situations errorList comes empty, so if that's the case we just do another .valid() that will generate the proper errorList
          let validate_data = $form.validate();
          if (!validate_data.errorList.length) {
            $form.valid();
            validate_data = $form.validate();
          }
          const ion_item = validate_data.errorList[0].element.closest('ion-item');
          const error_message = util.translate('exceptions.required') + ion_item.getAttribute('label');
          msg.showError(error_message);
          throw error_message;
        }
        //We know the form is valid
        checkRequiredLockedFields(edit);
        let tableName = tkn.objectConfig[edit.object].tableName;
        let props = $edit.find('[property]');
        let values = new Array();
        let fields = new Array();
        //We format properly the fields
        ...
        if (edit.filter) {
          ...
          resolve(sql.execSQL(UpdateScript, values).then(() => {
            if (goBack) {
              nav.goBack();
            }
            if (showMessage) {
          ...
          }).catch(err => { msg.showError(err); throw err; }));
        }
        else {
          let keys = tkn.objectConfig[edit.object].primaryKeys;
          for (let i = 0; i < keys.length; i++) {
            let val = $edit.find('[property="' + keys[i] + '"]');
          ...
          resolve(sql.execSQL(InserScript, values).then(() => {
            if (goBack) {
              nav.goBack();
            }
            if (showMessage) {
...
const caes = {
  login: {
    ...
    sleepMode: "En suspensió",
    loginWith: "Iniciar sessió amb",
    ssoNotSameAccount: "No coincideix el compte de l'ABH amb el compte de l'app sobre el qual esteu intentant iniciar sessió"
  },
  menu: {
    menu: 'Menú',
    home: 'Inici',
    sync: 'Sync', syncMore: 'Més opcions',
    logout: 'Eixida',
    trueLogout: 'Tancar la sessió',
  ...
  sync: {
    ...
    hasChanges: 'Canvis pendents',
    hasChangesSure: 'Hi ha canvis pendents, vols tancar la sessió de totes maneres?',
  ...
  indexedDB: {
    ...
    profilerFilter: 'Filtre del profiler'
  },
  flxIA: {
    firstMessage: 'Hola! Com puc ajudar-te?',
    inputPlaceholder: 'Pregunti lo que necesitis...',
    executing: "Executant procés",
    processSuccess: "Procés executat exitosament",
    processError: "Hi ha hagut un error executant el procés",
    noTokenHeader: "Assistent no configurat",
    noTokenMessage: "No has configurat el Bearer Token, si us plau, revisa la documentació"
  },
  bluetooth: {
    notSupported: "Aquest dispositiu no té bluetooth",
    notAuthorized: "L'autorització per al bluetooth ha estat rebutjada",
    notActive: "El bluetooth es troba apagat",
    noDevices: "No s'ha detectat cap dispositiu"
  }
};
...
const frfr = {
  login: {
    ...
    sleepMode: "En mode veille",
    loginWith: "Se connecter avec",
    ssoNotSameAccount: "Le compte ABH ne correspond pas au compte de l'application sur laquelle vous essayez de vous connecter"
  },
  menu: {
    menu: 'Menu',
    home: 'Début',
  ...
  sync: {
    ...
    hasChanges: 'Changements en cours',
    hasChangesSure: 'Des modifications sont en cours, souhaitez-vous toujours vous déconnecter?',
  ...
  indexedDB: {
    ...
    profilerFilter: 'Filtre profiler'
  },
  flxIA: {
    firstMessage: "Bonjour! Comment puis-je vous aider?",
    inputPlaceholder: 'Demandez quelque chose...',
    executing: "Processus d'exécution",
    processSuccess: "Le processus a été exécuté avec succès",
    processError: "Une erreur s'est produite lors de l'exécution du processus",
    noTokenHeader: "Assistant non configuré",
    noTokenMessage: "Vous n'avez pas configuré le jeton Bearer, veuillez vérifier la documentation"
  },
  bluetooth: {
    notSupported: "Cet appareil ne supporte pas bluetooth",
    notAuthorized: "L'autorisation Bluetooth n'a pas été donnée",
    notActive: "Bluetooth n'est pas actif",
    noDevices: "Aucun appareil n'a été détecté"
  }
};
...
const eses = {
  login: {
    ...
    sleepMode: "En suspensión",
    loginWith: "Iniciar sesión con",
    ssoNotSameAccount: "No coincide la cuenta del ABH con la cuenta de la app sobre la que estás intentando iniciar sesión"
  },
  menu: {
    menu: 'Menú',
    home: 'Inicio',
  ...
  sync: {
    ...
    hasChangesSure: 'Hay cambios pendientes ¿estás seguro de cerrar sesión?',
  ...
  indexedDB: {
    ...
    profilerFilter: 'Filtro del profiler'
  },
  flxIA: {
    firstMessage: '¡Hola! ¿Cómo puedo ayudarte?',
    inputPlaceholder: 'Pregunta lo que necesites...',
    executing: "Ejecutando proceso",
    processSuccess: "Proceso ejecutado con éxito",
    processError: "Hubo un error ejecutando el proceso",
    noTokenHeader: "Asistente no configurado",
    noTokenMessage: "No tienes configurado el Bearer Token, por favor revisa la documentación"
  },
  bluetooth: {
    notSupported: "Este dispositivo no tiene bluetooth",
    notAuthorized: "La autorización para el bluetooth ha sido rechazada",
    notActive: "El bluetooth se encuentra apagado",
    noDevices: "No se ha detectado ningún dispositivo"
  }
};
...
const dede = {
  login: {
    ...
    sleepMode: "Im Ruhemodus",
    loginWith: "Anmeldung mit",
    ssoNotSameAccount: "ABH-Konto stimmt nicht mit dem Konto der App überein, bei der Sie sich anzumelden versuchen"
  },
  menu: {
    menu: 'Menü',
    home: 'Start',
  ...
  sync: {
    ...
    hasChanges: 'Ausstehende Änderungen',
    hasChangesSure: 'Es stehen noch Änderungen aus, möchten Sie sich trotzdem abmelden?',
  ...
  indexedDB: {
    ...
    profilerFilter: 'Filter profiler'
  },
  flxIA: {
    firstMessage: 'Hallo!\nWie kann ich Ihnen helfen?',
    inputPlaceholder: 'Bitten Sie um etwas...',
    executing: "Ausführender Prozess",
    processSuccess: "Prozess erfolgreich ausgeführt",
    processError: "Bei der Ausführung des Prozesses ist ein Fehler aufgetreten",
    noTokenHeader: "Assistent nicht konfiguriert",
    noTokenMessage: "Sie haben das Berear-Token nicht konfiguriert, überprüfen Sie die Dokumentation"
  },
  bluetooth: {
    notSupported: "Dieses Gerät unterstützt kein Bluetooth",
    notAuthorized: "Bluetooth-Autorisierung wurde nicht erteilt",
    notActive: "Bluetooth ist nicht aktiv",
    noDevices: "Es wurde kein Gerät gefunden"
  }
};
...
const engb = {
  login: {
    ...
    sleepMode: "In sleep mode",
    loginWith: "Login with",
    ssoNotSameAccount: "ABH account does not match the account of the app you are trying to log in on"
  },
  menu: {
    menu: 'Menu',
    home: 'Home',
  ...
  sync: {
    ...
    hasChangesSure: 'There are changes pending, do you still want to logout?',
  ...
  indexedDB: {
    ...
    profilerFilter: 'Profiler filter'
  },
  flxIA: {
    firstMessage: 'Hello! How can I help you?',
    inputPlaceholder: 'Ask for something...',
    executing: "Executing process",
    processSuccess: "Process executed successfully",
    processError: "There was an error on process execution",
    noTokenHeader: "Assisntant not configured",
    noTokenMessage: "You haven't configured the Bearer Token, please check the documentation"
  },
  bluetooth: {
    notSupported: "This device does not support bluetooth",
    notAuthorized: "Bluetooth authorization was not given",
    notActive: "Bluetooth is not active",
    noDevices: "No device was detected"
  }
};
...
const ptpt = {
  login: {
    ...
    enterPassword: 'Introduzir a palavra-passe',
    sleepMode: "Em modo de espera",
    loginWith: "Faça login com",
    ssoNotSameAccount: "A conta ABH não corresponde à conta do aplicativo no qual você está tentando fazer login"
  },
  menu: {
    menu: 'Menu',
    home: 'Início',
  ...
  sync: {
    ...
    hasChangesSure: 'Há alterações pendentes, você ainda deseja fazer logout?',
  ...
  indexedDB: {
    ...
    profilerFilter: 'Filtro profiler'
  },
  flxIA: {
    firstMessage: '¡Olá! Como posso te ajudar?',
    inputPlaceholder: 'Peça algo...',
    executing: "Execução do processo",
    processSuccess: "Processo executado com êxito",
    processError: "Ocorreu um erro ao executar o processo",
    noTokenHeader: "Assistente não configurado",
    noTokenMessage: "Você não configurou o Token Bearer, por favor, verifique a documentação"
  },
  bluetooth: {
    notSupported: "Este dispositivo não é compatível com bluetooth",
    notAuthorized: "A autorização do Bluetooth não foi fornecida",
    notActive: "O Bluetooth não está ativo",
    noDevices: "Nenhum dispositivo foi detectado"
  }
};
...
const itit = {
  login: {
    ...
    sleepMode: "In modalità sospensione",
    loginWith: "Accesso con",
    ssoNotSameAccount: "L'account ABH non corrisponde all'account dell'app su cui si sta cercando di effettuare il login"
  },
  menu: {
    menu: 'Menu',
    home: 'Home',
  ...
  sync: {
    ...
    hasChanges: 'Modifiche in sospeso',
    hasChangesSure: 'Ci sono modifiche in attesa, si vuole ancora effettuare il logout?',
    manageAccounts: 'Gestione dei conti'
  },
  image: {
    delete: 'Elimina',
  ...
  flxIA: {
    ...
    inputPlaceholder: 'Chiedere qualcosa...',
    executing: "Esecuzione del processo",
    processSuccess: "Il processo è stato eseguito correttamente",
    processError: "Si è verificato un errore nell'esecuzione del processo",
    noTokenHeader: "Assistente non configurato",
    noTokenMessage: "Non hai configurato il Token Bearer, controlla la documentazione"
  },
  bluetooth: {
    notSupported: "Questo dispositivo non supporta il Bluetooth",
    notAuthorized: "L'autorizzazione Bluetooth non è stata data",
    notActive: "Il Bluetooth non è attivo",
    noDevices: "Non è stato rilevato alcun dispositivo"
  }
};
...
const Haptics = registerPlugin('Haptics', {
    web: () => import('./web-48c8e947.js').then(m => new m.HapticsWeb()),
});
...
(function (ext) {
  async function linkExternalFunctions() {
    ...
    window.flexygo.ai = ai;
    window.flexygo.zebraPrinter = zebraPrinter;
    window.flexygo.haptics = Haptics;
    window.flexygo.haptics.impactStyle = ImpactStyle;
    window['$'] = jquery;
  }
...
const App = registerPlugin('App', {
    web: () => import('./web-5426055e.js').then(m => new m.AppWeb()),
});
        ...
        if (!hmrVersionId || !BUILD.hotModuleReplacement) {
          ...
          switch(bundleId) {
                ...
                case 'jeep-sqlite':
                    return import(
                      /* webpackMode: "lazy" */
                      './jeep-sqlite.entry-4bfbad41.js').then(processMod, consoleError);
          }
      }
  return import(
    /* @vite-ignore */
  ...
  ConftokenProvider.config().then(async (conf) => {
    try {
      ...
      if (auth_code) {
        ...
        //We get the flx-login component so we can use loadingStart and selectApp methods
        const login_element = document.querySelector('flx-login');
        const accountlist_element = document.querySelector('flx-accountslist');
        //If there's a flx-accountslist it means its login through this menu, so we do not have to generate a new token, and we need to verify it's 
        //not getting lo logged through another account not corresponding to its selected app user
        if (accountlist_element) {
          let sso_auth_token = await accountlist_element.getActiveToken();
          sso_auth_token.sso_token = auth_code;
          await web_api.getSSOToken(sso_auth_token);
          const context = await web_api.get(sso_auth_token.url + '/webapi/context', null, { 'Authorization': 'Bearer ' + sso_auth_token.bearerToken });
          if (context.currentUserLogin !== sso_auth_token.user) {
            msg.showError(util.translate('login.ssoNotSameAccount'));
          }
          else {
            accountlist_element.loginActiveUser();
          }
          return;
        }
        login_element.loadingStart();
        //We store the token with the basic data we've got
        let auth_token = new authToken();
        auth_token.url = await storage$1.get('login_by_sso_url'); //This is the URL that we will have had stored after clicking login by SSO
        auth_token.aad_url = await storage$1.get('login_by_sso_aad_url');
        auth_token.sso_token = auth_code;
        const app_guid = util.GUID();
        await web_api.saveAuth(auth_token, app_guid);
        //We get the bearer token through this call, which also stores it on 
        auth_token = await web_api.getSSOToken(auth_token).catch(err => {
          login_element.loadingFinish();
          throw err;
        });
        //We get the UserName from the recently gotten token so when we the user tries to login again it doesn't cause an error
        //the error was caused due to not being able of comparing if that DB already exists due to a missing username
        const context = await web_api.get(auth_token.url + '/webapi/context', null, { 'Authorization': 'Bearer ' + auth_token.bearerToken });
        auth_token.user = context.currentUserLogin;
        await web_api.saveAuth(auth_token);
        //We use the flx-login selectApp method to navigate to the app or show the apps choosing menu
        web_api.getCollection('sysOfflineApp').then((apps) => {
          login_element.selectApp(apps, app_guid, auth_token);
        });
      }
      else {
        conf.urlConfig.url = url.searchParams.get('url');
        conf.urlConfig.user = url.searchParams.get('usr');
      }
...
export { ImpactStyle as I, NotificationType as N, getElement as a, createEvent as c, globalScripts as g, registerInstance as r };
