...
<head runat="server">
    ...
    <!-- CSS -->
    <%= Me.getCssScripts()%>
    <%= System.Web.Optimization.Styles.Render("~/bnd-" & FLEXYGO.Web.GlobalVars.VersionStr & "-" & Me.getBundleGUID() & "/plugins/css").ToHtmlString()%>
    <%= System.Web.Optimization.Styles.Render("~/bnd-" & FLEXYGO.Web.GlobalVars.VersionStr & "-" & Me.getBundleGUID() & "/" & IIf(FLEXYGO.Utilities.General.Util.IsBlank(FLEXYGO.Web.GlobalVars.ConfToken.UserSecurity.Mode), "light", FLEXYGO.Web.GlobalVars.ConfToken.UserSecurity.Mode).ToString() & "/css").ToHtmlString()%>
    <!--Skin CSS-->
    <%= Me.getSkinCss()%>
    <!--Interface CSS-->
    <%= Me.getInterfaceCss()%>
    ...
    <%= System.Web.Optimization.Scripts.Render("~/bnd-" & FLEXYGO.Web.GlobalVars.VersionStr & "-" & Me.getBundleGUID() & "/plugins/js").ToHtmlString()%>
  
    <%= System.Web.Optimization.Scripts.Render("~/bnd-" & FLEXYGO.Web.GlobalVars.VersionStr & "-" & Me.getBundleGUID() & IIf(FLEXYGO.Web.GlobalVars.ConfToken.UserSecurity.IsDesigner, "/a", "").ToString & "/js").ToHtmlString()%>
     <!--Skin JS-->
    <%= Me.getSkinScripts()%>
    <!--Interface JS-->
