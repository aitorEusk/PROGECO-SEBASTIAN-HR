...
(function (flexygo) {
    ...
    (function (utils) {
        ...
        function showQR(text) {
            ...
            const id = utils.uniqueUUID();
            let qr_container = document.createElement('div');
            qr_container.style.width = '400px';
            qr_container.style.height = '400px';
            qr_container.style.margin = '0 auto';
            qr_container.id = `lastQR-${id}`;
            let canvas = document.createElement('canvas');
            qr_container.appendChild(canvas);
            Lobibox.window({ title: 'QR Code', content: qr_container });
            QRCode.toCanvas(canvas, text, { width: 400, errorCorrectionLevel: 'H' }, function (error) { if (error)
                flexygo.msg.error(error); });
        }
        utils.showQR = showQR;
        function generateQR(text, size = 400) {
            // Creates a container for the QR
            let qr_container = document.createElement('div');
            qr_container.style.width = size + 'px';
            qr_container.style.height = size + 'px';
            qr_container.style.margin = '0 auto';
            // Creates a canvas element to render the QR
            let canvas = document.createElement('canvas');
            qr_container.appendChild(canvas);
            // Generates the QR
            QRCode.toCanvas(canvas, text, { width: size, errorCorrectionLevel: 'H' }, function (error) { if (error)
                flexygo.msg.error(error); });
            // We return the qr as a b64
        ...
        function copyClipboard(text) {
            return __awaiter(this, void 0, void 0, function* () {
                if (flexygo.utils.isBlank(text))
                    return false;
                const msgKey = 'msg.copied';
                let msg = flexygo.localization.translate(msgKey);
                if (msg === msgKey) {
                    msg = "Copied";
                }
                try {
        ...
        */
        ...
        /**
         * Returns the name of the OS being used, if we don't recognize it we return null
         * @method getOs
         * @returns {string|null} Returns "WINDOWS", "MACOS", "LINUX", "ANDROID", "IOS" or null
         */
        function getOS() {
       ...
       */
        ...
        */
        ...
        function randomColorClass(seed) {
            let classList = ['outstanding', 'success', 'notify', 'danger', 'warning', 'primary', 'tools'];
            let hash = 0;
            for (let i = 0; i < seed.length; i++) {
                hash = (hash << 5) - hash + seed.charCodeAt(i);
            }
            let numero = Math.abs(hash) % 7;
            return `flx-state-${classList[numero]}`;
        }
        utils.randomColorClass = randomColorClass;
    ...
    */
        ...
        utils.showLoading = (element, text, position, zIndex) => {
            var _a;
            if (isBlank(element)) {
                element = $('#mainContent');
            }
        ...
        utils.showLoadingEffect = (time = 5000, element = null, text, position, noTimer = false, zIndex) => {
            utils.showLoading(element, text, position, zIndex);
            if (!noTimer) {
                setTimeout(function () {
        ...
        function getZIndexElement(elm) {
            ...
            if (elm != undefined && $(elm)[0] !== $(document)[0]) {
                ...
                if (isNaN(zindex)) {
                    if ($(elm).parent()[0] instanceof Element) {
            ...
            if (codeBlocks) {
                codeBlocks.forEach((codeBlock) => {
                    ...
                    if (language === 'chart') {
                        ...
                        message = message.replace(codeBlock, content);
                    }
                    else {
...
function setEditsValues(objDoc) {
    ...
    print_edits.each((_, current_print_edit) => {
        ...
        form_controls.each((_, control) => {
            ...
            //We set the value on the control depending on its type is date
            ...
            switch (print_property.nodeName) {
                ...
                case 'FLX-COMBO':
                    ...
                    break;
                default:
                    switch (print_property.type) {
                        case 'date':
                            //print_property.setValue(value.toLocaleDateString());
                            $(print_property).find('input').val(moment(value).format("YYYY-MM-DD"));
                            break;
                        default:
    ...
    (function (utils) {
        ...
        (function (geTablesChangeTemplate) {
            ...
            function showChanges(button) {
                ...
                const module = button.closest('#realMain, main').querySelector('flx-module[modulename="sysMod_GetTablesChanges"]');
            ...
            function executeLoadedModuleFunctions(module) {
                let list = module.querySelector('#mod-sysMod_GetTablesChanges');
                //We check the checkboxes which value is true, because there's no other way of setting them
                ...
                if (list.querySelector('td'))
                    module.closest('main').querySelector('button.pressableBtn.warning').classList.remove('disabled');
            }
            ...
            function generateScript(button) {
                ...
                let tables = '';
                const realMain = button.closest('#realMain, main');
        ...
        (function (modules) {
            ...
            function removeSkeleton(module) {
                ...
                if (module.skeleton) {
                    ...
                    if ((_a = module.skeleton_data) === null || _a === void 0 ? void 0 : _a.scroll) {
                        module.skeleton_data.element.scrollTop = module.skeleton_data.scroll;
                    }
                    (_b = module.querySelector('.flxSkeleton')) === null || _b === void 0 ? void 0 : _b.remove();
                    (_c = module.querySelector('.cntBody')) === null || _c === void 0 ? void 0 : _c.classList.remove('displayingSkeleton');
                }
            }
            ...
            function createLabelProperty(poperty_name, label) {
                return {
                    _SQlSentence: "",
                    _SQLEditSentence: "",
                    DropDownRows: 0,
                    CanView: true,
                    CanEdit: true,
                    CanView_ComboObject: true,
                    CanEdit_ComboObject: true,
                    CanInsert_ComboObject: true,
                    Key: false,
                    ObjectName: "Location",
                    HasDefinition: true,
                    Name: poperty_name,
                    ...
                    SearchTypes: "text",
                    Mask: "",
                    SQLSentence: "d5An7VKFmyY=",
                    SQLEditSentence: "",
                    SQLFilter: "d5An7VKFmyY=",
                    SQLDisplayField: "",
                    SQLValueField: "",
                    SQLObjectName: "",
                    SQLViewName: "",
                    ...
                    DataTypeSQL: 0,
                    WebComponent: "flx-separator",
                    OfflineWebComponent: "ion-input type=\"text\" inputmode=\"text\"",
                    IconClass: "",
                    IconName: "",
                    ToolbarName: null,
                    DropDownValues: null,
                    ...
                    ImageCompressionType: 0,
                    ImageMaxWidth: 0,
                    ImageMaxHeight: 0,
                    ExtensionId: "sysWhiteList",
                    Extensions: ".xbm|.tif|.pjp|.svgz|.jpg|.jpeg|.ico|.tiff|.gif|.svg|.jfif|.webp|.png|.bmp|.pjpeg|.avif|.doc|.docx|.pdf|.txt|.xlsx|.avi|.mp4|.mp3|.wav|.wma|.zip|.rar|.mpeg|.mwv|.ppt|.csv|.pptx|.pptm|.xls|.xlt|.xltx|.xlsb|.xlsm|.xml|.json",
                    Autoselect: false,
                    HasDependencies: false,
                    HasDependingProperties: false,
                    ThrowDependenciesOnInvalid: true,
                    BarcodeReaders: null,
                    ComboAllowSave: false,
                    ComboAllowSave_Object: null,
                    ComboAllowSave_DisplayField: null,
                    ComboAllowSave_ValueField: null,
                    ComboAllowSave_Defaults: null,
                    ComboAllowSave_WarningMessage: false,
                    ComboAllowSave_CanInsert: false,
                    CustomPropName: null,
                    IsCustomControl: false,
                    DescriptionInApi: null,
                    ShowValuesInApi: false
                };
            }
            modules.createLabelProperty = createLabelProperty;
