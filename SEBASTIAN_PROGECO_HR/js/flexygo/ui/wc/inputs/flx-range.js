            ...
            class FlxRangeElement extends HTMLElement {
               ...
               */
                ...
                init() {
                    ...
                    // We check if the device is IOS or an iPad so we can change it's style accordingly to avoid over
                    const is_ios_ipad = /iPad|iPhone|iPod/.test(navigator.platform) || (navigator.platform === 'MacIntel' && navigator.maxTouchPoints > 1);
                    if (is_ios_ipad) {
                        this.classList.add("is-ios-ipad");
                    }
                    let control = $('<div>');
