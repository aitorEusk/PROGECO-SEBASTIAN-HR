...
(function (flexygo) {
    ...
    (function (ui) {
        ...
        (function (wc) {
            ...
            class FlxDbComboElement extends HTMLElement {
               ...
               */
                connectedCallback() {
                    ...
                    let TargetIdAllowNew = element.attr('TargetIdAllowNew');
                    if (TargetIdAllowNew && TargetIdAllowNew !== '') {
                        if (!this.options) {
                            this.options = new flexygo.api.ObjectProperty();
                        }
                        this.options.TargetIdAllowNew = TargetIdAllowNew;
                    }
               ...
               */
                attributeChangedCallback(attrName, oldVal, newVal) {
                    ...
                    if (attrName.toLowerCase() === 'objwherelink' && newVal) {
                        this.options.ObjWhereLink = newVal;
                    }
                    if (attrName.toLowerCase() === 'allownewdefaults' && newVal) {
                        this.options.AllowNewDefaults = newVal;
                    }
                }
                refresh() {
                ...
                initEditMode() {
                    ...
                    if (this.options) {
                        ...
                        input.off('focus').on('click', (ev) => {
                            ...
                            if (!new_value) {
                                this.removeAttribute("addValuesToObject");
                            }
                            me.attr('value', new_value);
                            e.stopPropagation();
                            e.preventDefault();
                        });
                        if (!flexygo.utils.isSizeMobile() && !flexygo.utils.isTactilModeActive()) {
                        ...
                        if (navigator.userAgent.toLowerCase().indexOf('firefox') > -1) {
                            me.off('mouseenter.comboSearch').on('mouseenter.comboSearch', function () {
                                var _a;
                                if (!flexygo.utils.isBlank(me.find('input[type="search"]').val())) {
                                    (_a = me.find('.input-group-btn > button.icon-close')) === null || _a === void 0 ? void 0 : _a.removeClass('hidden');
                                }
                            });
                            me.off('mouseleave.comboSearch').on('mouseleave.comboSearch', function () {
                                var _a;
                                (_a = me.find('.input-group-btn > button.icon-close')) === null || _a === void 0 ? void 0 : _a.addClass('hidden');
                            });
                            me.find('button.icon-close').off('click.clearSearch').on('click.clearSearch', function () {
                                var _a;
                                me[0].setValue('');
                                me.find('ul li.selected').removeClass('selected');
                                (_a = me.find('.input-group-btn > button.icon-close')) === null || _a === void 0 ? void 0 : _a.addClass('hidden');
                                me.find('input[name="DbComboField"]').trigger('click');
                    ...
                    if (!this.open && !this.input.prop('readonly')) { //!this.datalist.is(':visible')
                        if (!this.mobileInput) {
                            ...
                            if (is_closer_to_top) {
                                ...
                                this.datalist.css({ top: top_position, bottom: 'unset', left: position_data.x, width: position_data.width, 'max-height': window.innerHeight - top_position - max_height_margin });
                            }
                            else {
                                const bottom_position = window.innerHeight - position_data.y;
                                this.datalist.css({ top: 'unset', bottom: bottom_position, left: position_data.x, width: position_data.width, 'max-height': window.innerHeight - bottom_position - max_height_margin });
                            }
                ...
                loadValues(page, autoselect, fromvalue, value, append) {
                    ...
                    if (this.options.ViewName && this.options.ViewName !== '') {
                        params = {
                            ...
                            Page: page,
                            PageSize: (_a = this.options.DropDownRows) !== null && _a !== void 0 ? _a : null,
                ...
                addComboItems(data, autoselect, append) {
                    ...
                    if (this.options.ComboAllowSave && this.datalist.find('.newValueField').length === 0 && (!data || !data.some(el => (el[this.options.SQLDisplayField] + "").toLowerCase() === (input_value + "").toLowerCase()))) {
                        //Falta traducir
                        let new_value_field = $(`<li class="newValueField ${input_value ? '' : 'hidden'} ${(data === null || data === void 0 ? void 0 : data.length) ? '' : 'selected'}">
                    ...
                    if (data) {
                        ...
                        const uses_sql = !!this.options.ViewName || !!this.options.SQLEditSentence || !!this.options.SQLSentence;
                        if (this.datalist.find(' > li').length >= (this.options.DropDownRows || flexygo.profiles.defaultDropDownRows) && data.length > 0 && uses_sql) {
                ...
                onClickItem(ev) {
                    ...
                    if (item.is('.newValueField')) {
                        value = item.find('.currentlyTyping').text();
                        text = value;
                        is_new = true;
                    }
                    else {
                    ...
                    this.triggerDependencies();
                    //If it's a new value we add the AddValuesToObject attribute to know that it contains a new value on save
                    if (is_new && this.options.ComboAllowSave_Object) {
                ...
                getIconButtons() {
                    ...
                    //In firefox
                    if (navigator.userAgent.toLowerCase().indexOf('firefox') > -1) {
                        let close = $('<button class="btn btn-default flx-icon icon-close hidden" type="button" tabindex="-1"></button>');
                        ret.append(close);
                    }
                    ret.append(arrow);
                    ...
                    if (this.options && (this.options.AllowNewFunction || this.options.AllowNewObject) && !this.options.Locked && this.options.CanInsert_ComboObject) {
                        icon1 = $('<button class="btn btn-default flxallownew" type="button"><i class="fa fa-plus" /></button>').on('click', () => {
                            if (this.options.AllowNewFunction) {
                                ...
                                if (!this.options.TargetIdAllowNew) {
                                    this.options.TargetIdAllowNew = 'modal';
                                }
                                flexygo.nav.openPage('edit', propAllowNewObject, null, propAllowNewDefaults, this.options.TargetIdAllowNew);
                            }
                        });
                        ret.append(icon1);
                    }
                ...
                setValueView(value, text) {
                    this.value = value;
                    $(this).find('label').text(text);
                    if (text !== null) {
                        $(this).find('label').attr('title', text);
                    }
                }
                getValue() {
                    let me = $(this);
           ...
           */
            FlxDbComboElement.observedAttributes = ['type', 'property', 'objectname', 'viewname', 'sqlvaluefield', 'sqldisplayfield', 'required', 'disabled', 'requiredmessage', 'style', 'class', 'placeholder', 'iconclass', 'template', 'helpid', 'hide', 'additionalwhere', 'sqlfilter', 'pagesize', 'validatormessage', 'cnnstring', 'allownewdefaults', 'objwherelink'];
