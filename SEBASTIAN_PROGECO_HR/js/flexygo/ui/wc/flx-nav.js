               ...
               */
                ...
                */
                loadNodesRet(ret) {
                    ...
                    if (this.mode === 'nav' && this.search) {
                        me.find('li.hidden').each((_, li) => {
                            $(li)
                                .addClass('hidden-initial')
                                .removeClass('hidden');
                        });
                        let searchWrapper = $(`<div class="nav-search-wrapper">
                                            <input type="text" class="nav-search" placeholder="${flexygo.localization.translate('flxsearch.search')}">
                                            <span class="nav-search-clear" aria-hidden="true"></span>
                                        </div>`);
                        let searchInput = searchWrapper.find('.nav-search');
                        let clearBtn = searchWrapper.find('.nav-search-clear');
                        searchInput.on('input', () => {
                            const value = searchInput.val();
                            searchWrapper.toggleClass('has-value', !!value);
                            this.filterNav(value);
                        });
                        clearBtn.on('click', () => {
                            searchInput.val('');
                        ...
                        case 'report': // Report Link
                            if (json.reportname && json.reportname != '') {
                                if (json.reporthasparams) {
                                    ...
                                    retFunction = flexygo.utils.functionToString('flexygo.nav.viewReport', [json.reportname, json.reportwhere, json.objectname, json.objectwhere, objDef, null, json.targetid], [false, false, json.print]);
                                }
                            }
                            else {
                                retFunction = "flexygo.msg.alert(flexygo.localization.translate('flxnav.pIdNotespecified'));";
               ...
               */
                ...
                */
                ...
                filterNav(term) {
                    ...
                    const rootUl = $(this).children('ul').first();
                    // Reset all <li> states and restore original text
                    rootUl.find('li').each((_, li) => {
                        const $li = $(li);
                        const $span = $li.find('.navTextSpan').first();
                        // Restore original text (remove <mark>)
                        if ($span.length) {
                            const original = $span.data('original-text') || $span.text();
                            $span.data('original-text', original);
                            $span.html(original);
                        }
                        // Reset visibility and submenu state
                        $li.removeClass('hidden-search');
                        if ($li.hasClass('hidden-initial')) {
                            $li.addClass('hidden-search');
                        }
                        $li.children('ul').hide();
                        $li.children('.item-opened')
                            .removeClass('item-opened')
                            .addClass('item-closed');
                    });
                    // If search is empty, stop here (menu fully reset)
                    if (!searchTerm)
                        return;
                    // Apply recursive filtering
                    this.filterList(rootUl, searchTerm);
                }
                filterList($ul, term) {
                    var _a;
                    const isDevMode = (_a = flexygo.debug) === null || _a === void 0 ? void 0 : _a.isDevelopMode();
                    ...
                    $ul.children('li').each((_, li) => {
                        ...
                        // Hide development-only items when not in dev mode
                        if ($li.hasClass('develop-only') && !isDevMode) {
                            $li.addClass('hidden-search');
                            return;
                        }
                        ...
                        if (childUl.length > 0) {
                            childHasMatch = this.filterList(childUl, term);
                        }
                        // Item is visible if it matches or any child matches
                        const visible = matches || childHasMatch;
                        $li.toggleClass('hidden-search', !visible);
                        // Highlight matched text (or reset if not matched)
                        this.highlightText($li, matches ? term : '');
                        if (visible)
                            anyVisible = true;
                        // Expand submenu if it contains matches
                        if (childHasMatch) {
                            childUl.show();
                            $li.children('.item-closed')
                                .removeClass('item-closed')
                                .addClass('item-opened');
                        }
                    });
                    // Return whether any item in this <ul> is visible
                    return anyVisible;
                }
                highlightText($li, term) {
                    const $span = $li.find('.navTextSpan').first();
                    if (!$span.length)
                        return;
                    // Get or store original text (without highlights)
                    const original = $span.data('original-text') || $span.text();
                    $span.data('original-text', original);
                    // If no term, restore original text
                    if (!term) {
                        if ($span.html() !== original) {
                            $span.html(original);
                        }
                        return;
                    }
                    // Escape term and highlight all occurrences
                    const regex = new RegExp(`(${term.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')})`, 'gi');
                    const highlighted = original.replace(regex, `<mark>$1</mark>`);
                    // Update DOM only if content changed
                    if ($span.html() !== highlighted) {
                        $span.html(highlighted);
                    }
                }
            }
            FlxNavElement.observedAttributes = ['mode', 'hierarchical', 'search'];
