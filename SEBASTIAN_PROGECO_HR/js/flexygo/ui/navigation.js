        ...
        function viewReport(reportname, reportwhere, objectname, objectwhere, defaults, params, targetid, excludeHist, hasParams = false, print = false) {
            ...
            flexygo.ajax.post('~/api/Report', 'GetReportConfig', { "ReportName": reportname, "ObjectName": objectname, "ObjectWhere": objectwhere }, (reportConfig) => {
                ...
                if (reportConfig.TypeId == 'html') {
                    ...
                    filter = flexygo.utils.parser.compile(paramsJSON, (reportConfig.AdditionalWhere == null ? "" : reportConfig.AdditionalWhere));
                    flexygo.ajax.syncPost('~/api/Report', 'GetReportExcel', { "ObjectName": objectname, "ObjectWhere": objectwhere, "ReportName": reportname, "AdditionalWhere": filter, "Params": params, "Defaults": defaults }, (ret) => {
                        if (ret.Success) {
                ...
                else if ((reportConfig.TypeId == 'crystal' || reportConfig.TypeId == "devexpress" || reportConfig.TypeId == "devexpressfile") && reportConfig.ReportMode == 'Pdf' && reportConfig.PrintId == 'print' && !flexygo.utils.isSizeMobile()) {
                    flexygo.utils.showLoading(null, flexygo.localization.translate('htmlreport.generate'));
                    ...
                    flexygo.nav.openURL('~/forms/Reports.aspx', callParams, '_self');
                }
            });
        }
        ...
        */
        function openURL(url, params, targetid) {
            ...
            if (flexygo.utils.isAgentMobile() || flexygo.utils.isAgentElectron()) {
                if (targetid == '_self') {
                    if (url.toLowerCase().indexOf('reports.aspx') != -1) {
                        flexygo.msg.info(flexygo.localization.translate('navigation.reportinfo'));
                    }
                    return window.open(url, targetid);
                }
                else {
       ...
       */
        function openHelpId(helpid, targetid, excludeHist, triggerElement) {
            ...
            if (targetid && targetid.indexOf('new') == 0) {
                ...
                const userLang = flexygo.context.currentUserCultureId;
                flexygo.ajax.post('~/api/Page', 'GetHelpById', { "HelpId": helpid, "UserLang": userLang }, (ret) => {
                    if (ret) {
                        ret.pageHistory = histObj;
                ...
                if (btn) {
                    ...
                    if (histObj != null && histObj.defaults) {
                        if (typeof histObj.defaults == 'string') {
                            ...
                            try {
                                defPage = JSON.parse(histObj.defaults);
                            }
                            catch (_a) {
