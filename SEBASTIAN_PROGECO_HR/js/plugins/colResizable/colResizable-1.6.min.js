...
(function($){ 	
	...
	h.append("<style type='text/css'>  .JColResizer{table-layout:fixed;} .JColResizer > tbody > tr > td{overflow:hidden;padding-left:4!important; padding-right:4!important;} .JColResizer > tbody > tr > th{overflow:hidden;padding-left:0!important; padding-right:0!important;},  .JCLRgrips{ height:0px; position:relative;} .JCLRgrip{margin-left:-5px; position:absolute; z-index:5; } .JCLRgrip .JColResizer{position:absolute;background-color:red;filter:alpha(opacity=1);opacity:0;width:10px;height:100%;cursor: e-resize;top:0px} .JCLRLastGrip{position:absolute; width:1px; } .JCLRgripDrag{ border-left:1px dotted black;	} .JCLRFlex{width:auto!important;} .JCLRgrip.JCLRdisabledGrip .JColResizer{cursor:default; display:none;}</style>");
	...
	var createGrips = function(t){	
        ...
        var gridSizes = flexygo.storage.local.get('gridSizes') || {};
        if (t.p && S && gridSizes[t.id])memento(t,th);		//if 'postbackSafe' is enabled and there is data for the current table, its coloumn layout is restored
	...
	var memento = function(t, th){ 
		...
		if(th){										//in deserialization mode (after a postback)
            ...
            if (t.opt.flush) { flexygo.storage.local.remove(t.id) = ""; return; } 	//if flush is activated, stored data is removed
            var gridSizes = flexygo.storage.local.get('gridSizes') || {};
            w = gridSizes[t.id].split(";");					//column widths is obtained
			tw = w[t.ln+1];
			...
			for(;i < t.c.length; i++){	//iterate through columns
                ...
                sizesTb += w+";";		//width is appended to the sessionStorage object using ID as key
				m+=w;					//carriage is updated to obtain the full size used by columns
			}
            sizesTb +=m;							//the last item of the serialized string is the table's active area (width), 
												//to be able to obtain % width value of each columns while deserializing
            if (!t.f) sizesTb += ";" + t.width(); 	//if not fixed, table width is stored
            var gridSizes = flexygo.storage.local.get('gridSizes') || {};
            gridSizes[t.id]=sizesTb;
