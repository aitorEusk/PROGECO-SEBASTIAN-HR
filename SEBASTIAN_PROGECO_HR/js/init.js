$(function () {
    ...
    flexygo.events.on(this, 'process', 'error', function (e) {
        if (document.body.querySelector(`flx-ai[settingid="${e.masterIdentity}"]`)) {
            flexygo.msg.warning(e.detailIdentity);
        }
    });
...
function initPage() {
    ...
    let goUpArrow = $('#flx-goUpArrow')[0];
    if (goUpArrow) {
        goUpArrow.title = flexygo.localization.translate('navigation.scrolltop');
    }
                ...
                Lobibox.confirm({
                    ...
                    callback: (dlg, type, ev) => {
                        if (type == "accept") {
                            ...
                            flexygo.nav.execProcess('ReloadQuietCache', '', '', null, null, 'current', false, $(this), null, false);
                        }
                    }
                });
            }
