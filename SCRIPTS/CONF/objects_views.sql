...
N'GROUP BY IdNotificacion",
	...
	"ConnStringId":"DataConnectionString",
	"SQLSentence":" SELECT [PERS_Notificaciones].[IdNotificacion], [FlxCmb1].[FullName] as [IdNotificacion_1], [PERS_Notificaciones].[Texto] as [Texto], [PERS_Notificaciones].[Titulo] as [Titulo], [FlxCmb2].[FullName] as [Inserted By], [FlxCmb3].[FullName] as [Update By], [PERS_Notificaciones].[Inserted] as [Inserted], [PERS_Notificaciones].[LastUpdate] as [Last Update], [FlxCmb4].[FullName] as [Empleado] FROM [PERS_Notificaciones] \n',
N'  LEFT JOIN (SELECT [EmployeeId] ,[Name] ,[Surname],[FullName],[Blocked], [Group] , UnitId \n',
N' FROM [Employees] ) [FlxCmb1] ON [FlxCmb1].[EmployeeId]=[PERS_Notificaciones].[IdNotificacion] \n',
N'  LEFT JOIN (SELECT [EmployeeId] ,[Name] ,[Surname],[FullName],[Blocked], [Group] , UnitId \n',
N' FROM [Employees] ) [FlxCmb2] ON [FlxCmb2].[EmployeeId]=[PERS_Notificaciones].[InsertedBy] \n',
N'  LEFT JOIN (SELECT [EmployeeId] ,[Name] ,[Surname],[FullName],[Blocked], [Group] , UnitId \n',
N' FROM [Employees] ) [FlxCmb3] ON [FlxCmb3].[EmployeeId]=[PERS_Notificaciones].[UpdateBy] \n',
N'  LEFT JOIN (SELECT [EmployeeId] ,[Name] ,[Surname],[FullName],[Blocked], [Group] , UnitId \n',
N' FROM [Employees] ) [FlxCmb4] ON [FlxCmb4].[EmployeeId]=[PERS_Notificaciones].[EmployeeId] \n',
N'\n',
N'",
	"NoFilter":false,
	"ShowAsGrid":true,
